...
void drive(int distance, float speed){
    tacho_set_speed_sp(MOTOR_RIGHT, max_driving_speed * speed * DRIVING_CORRECTION);    // Set speed for the right motor with a slight driving correction constant 
    tacho_set_speed_sp(MOTOR_LEFT, max_driving_speed * speed);  // Set speed for the left motor 

    tacho_set_position_sp(MOTOR_RIGHT, distance * PULSE_PER_MM);    // Set a new position for the right motor
    tacho_set_position_sp(MOTOR_LEFT, distance * PULSE_PER_MM);    // Set a new position for the left motor

    tacho_run_to_rel_pos(MOTOR_BOTH);                   // Run the motors to the latest set position
...
void from_wall(int distance, float speed){
    ...
    tacho_run_forever(MOTOR_BOTH);  // Drive
    while (sensor_get_value0(sensor_us, 0) < distance); // hold as long as the distance from the wall is lower than set distance 
